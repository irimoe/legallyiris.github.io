...
const toggleOpen = () => {
	...
	if (opened) {
		isOpen.value = opened
		const firstItem = document.querySelector(`#optionlist-${randomId.value} li`)
...
const handleKeyDown = (event: KeyboardEvent) => {
	if (isOpen.value) {
		...
		const listItems = document.querySelectorAll(`#optionlist-${randomId.value} li`)
		...
		toggleOpen()
		nextTick(() => {
			const firstItem = document.querySelector(`#optionlist-${randomId.value} li:first-child`)
			if (!firstItem) return
...
const handleBlur = (event: FocusEvent) => {
	...
	if (!relatedTarget || !relatedTarget.closest(`#optionlist-${randomId.value}`)) {
		isOpen.value = false
	}
}
